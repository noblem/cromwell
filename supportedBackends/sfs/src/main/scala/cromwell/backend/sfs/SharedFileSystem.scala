...
trait SharedFileSystem extends PathFactory {
  ...
  def mapJobWomFile(job: JobPaths)(womFile: WomFile): WomFile = {
    val hostPath = hostAbsoluteFilePath(job.callExecutionRoot, womFile.containerPath)

    if (!hostPath.exists) throw new RuntimeException("Could not process output, file not found: " +
      s"${hostAbsoluteFilePath(job.callExecutionRoot, womFile.containerPath).pathAsString}")

    womFile.assignContainerPath(Function.const(hostPath.pathAsString))
  }
